...
class StockReport(BaseModel):
    ...
    price_trend_analysis: str = Field(..., description="价格趋势分析（含支撑/压力位、量价关系）", max_length=500)
    ...
    capital_flow_analysis: str = Field(..., description="资金动向分析（主力/北向/融资融券,markdown格式）", min_length=50,
                                       max_length=200)
    risk_assessment: str = Field(..., description="风险评估（波动率、Beta、VaR量化,markdown格式）", min_length=50,
                                 max_length=200)
    weekly_forecast: str = Field(..., description="未来一周预测（概率分布、目标价位,markdown格式）", min_length=50,
                                 max_length=200)
