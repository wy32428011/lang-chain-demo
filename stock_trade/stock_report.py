...
class StockReport(BaseModel):
    ...
    technical_indicators: str = Field(..., description="技术指标解读（MA5/10、MACD、RSI量化分析,markdown格式）", min_length=2000)
    news_sentiment_analysis: str = Field(..., description="新闻情绪分析（情感极性量化、政策影响,markdown格式）", min_length=2000)
    industry_comparison: str = Field(..., description="行业对比分析（相对强弱、同行业对比,markdown格式）", min_length=2000)
    capital_flow_analysis: str = Field(..., description="资金动向分析（主力/北向/融资融券,markdown格式）", min_length=2000)
    ...
    investment_rating: Literal["强烈买入", "买入", "中性", "卖出", "强烈卖出"] = Field(..., description="投资评级：强烈买入、买入、中性、卖出、强烈卖出")
